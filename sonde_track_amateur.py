...
def on_message_amateur(msg):
    print(json.dumps(msg, indent=2))
    if cfg['callsign'] == msg['payload_callsign']:
        print(json.dumps(msg, indent=2))
        global cnt
        cnt = cnt + 1
        ts = datetime.datetime.utcnow()
        rae = razel.RAZEL(obs['lat'], obs['lon'], obs['alt']/1000.0,
                          msg['lat'], msg['lon'], msg['alt']/1000.0)
        print("MESSAGE INFO:", msg['payload_callsign'])
        print("        counter: {:d}".format(cnt))
        print("   Sonde Serial:", msg['payload_callsign'])
        print(" Sonde Datetime:", msg['datetime'])
        print("  Received Time:", msg['time_received'])
        print("            Now:", datetime.datetime.strftime(ts, "%Y-%m-%dT%H:%M:%S.%fZ"))
        print("      Sonde Loc:", msg['lat'], msg['lon'], msg['alt'])
        ...
        print("   Altitude [km]: {:3.3f}".format(msg['alt']/1000.0))
        # print("V Velocity [m/s]: {:+3.3f}".format(msg['vel_v']))
        if msg['ascent_rate'] < 0.0:
            print("V Velocity [m/s]: {:+3.3f}".format(msg['ascent_rate']), "DESCENDING")
        else:
            print("V Velocity [m/s]: {:+3.3f}".format(msg['ascent_rate']), "Ascending")
        # print("H Velocity [m/s]: {:3.3f}".format(msg['vel_h']))
        # print("   Heading [deg]: {:3.3f}".format(msg['heading']))
        # if 'temp' in msg.keys(): 
        #     print(" Temperature [C]: {:3.3f}".format(msg['temp']))
...
def on_message_amateur_simple(msg):
    # print(json.dumps(msg, indent=2))
    req_keys = ['payload_callsign', 'datetime', 'lat', 'lon', 'alt']
    # print(list(msg.keys()))
    if all(item in list(msg.keys()) for item in req_keys):
        # print('woot')
        global cnt
        cnt = cnt + 1
        ts = datetime.datetime.utcnow()
        ...
        print("MESSAGE INFO:", msg['payload_callsign'])
        print("        counter: {:d}".format(cnt))
        print(" Sonde Callsign:", msg['payload_callsign'])
        print(" Sonde Datetime:", msg['datetime'])
        print("  Received Time:", msg['time_received'])
        print("            Now:", datetime.datetime.strftime(ts, "%Y-%m-%dT%H:%M:%S.%fZ"))
        print("      Sonde Loc:", msg['lat'], msg['lon'], msg['alt'])
...
if __name__ == '__main__':
    ...
    cfg.add_argument('--cfg_file',
                       ...
                       action="store")
    cfg.add_argument('--callsign',
                       dest='callsign',
                       type=str,
                       default=None,
                       help="Amateur Balloon Callsign",
                       action="store")
    ...
    if not args.callsign:
        print("Please enter a target Amateur Balloon Callsign...")
        sys.exit()
    ...
    test = sondehub.Stream(on_message=on_message_amateur_simple, 
                           prefix="amateur")
    while 1:
        time.sleep(1)
        pass
